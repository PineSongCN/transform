_N_E = ("object" !== typeof self
  ? self
  : (this.webpackJsonp_N_E =
      "object" !== typeof self ? self : this.webpackJsonp_N_E || [])
).push([
  [68],
  {
    LzRv: function(t, e, n) {
      "use strict";
      n.r(e),
        n.d(e, "default", function() {
          return m;
        });
      var r = n("nKUr"),
        a = n("vJKn"),
        o = n.n(a),
        s = n("rg98"),
        u = n("xvhg"),
        c = n("sN8P"),
        i = n("q1tI"),
        l = n("3o63"),
        f = n("xC0i"),
        p = [
          {
            type: l.a.SELECT,
            key: "property_name_format",
            label: "Property Name Format",
            options: [
              "PascalCase",
              "camelCase",
              "snake_case",
              "SCREAMING_SNAKE_CASE",
              "kebab-case",
              "SCREAMING-KEBAB-CASE",
              "UPPERCASE"
            ].map(function(t) {
              return { label: t, value: t };
            })
          }
        ],
        _ = { property_name_format: "camelCase" };
      function m() {
        var t = Object(f.a)("json-to-rust-serde", _),
          e = Object(u.a)(t, 2),
          a = e[0],
          m = e[1],
          b = Object(i.useCallback)(
            (function() {
              var t = Object(s.a)(
                o.a.mark(function t(e) {
                  var r, s, u;
                  return o.a.wrap(function(t) {
                    for (;;)
                      switch ((t.prev = t.next)) {
                        case 0:
                          return (
                            (r = e.value),
                            (t.next = 3),
                            Promise.all([n.e(10), n.e(11)]).then(
                              n.bind(null, "P0Z9")
                            )
                          );
                        case 3:
                          return (
                            (s = t.sent),
                            (u = s.run),
                            t.abrupt(
                              "return",
                              u(
                                "Root",
                                r,
                                JSON.stringify({
                                  output_mode: "rust",
                                  property_name_format: a.property_name_format
                                })
                              )
                            )
                          );
                        case 6:
                        case "end":
                          return t.stop();
                      }
                  }, t);
                })
              );
              return function(e) {
                return t.apply(this, arguments);
              };
            })(),
            [a]
          ),
          g = Object(i.useCallback)(function(t) {
            var e = t.open,
              n = t.toggle;
            return Object(r.jsx)(l.b, {
              title: "JSON to Rust Serde",
              onSubmit: m,
              open: e,
              toggle: n,
              formsFields: p,
              initialValues: a
            });
          }, []);
        return Object(r.jsx)(c.a, {
          transformer: b,
          editorTitle: "JSON",
          editorLanguage: "json",
          resultTitle: "Rust Serde",
          resultLanguage: "rust",
          resultSettingsElement: g,
          settings: a
        });
      }
    },
    teuK: function(t, e, n) {
      (window.__NEXT_P = window.__NEXT_P || []).push([
        "/json-to-rust-serde",
        function() {
          return n("LzRv");
        }
      ]);
    },
    xC0i: function(t, e, n) {
      "use strict";
      n.d(e, "a", function() {
        return s;
      });
      var r = n("q1tI"),
        a = n("kiQV"),
        o = "transform:".concat(a.version, ":");
      function s(t, e) {
        return (function(t, e) {
          var n = Object(r.useState)(function() {
              try {
                var n = window.localStorage.getItem(o + t) || e;
                return JSON.parse(n);
              } catch (r) {
                return e;
              }
            }),
            a = n[0],
            s = n[1];
          return [
            a,
            function(e) {
              try {
                var n = e instanceof Function ? e(a) : e;
                s(n), window.localStorage.setItem(o + t, JSON.stringify(n));
              } catch (r) {
                console.log(r);
              }
            }
          ];
        })("settings:".concat(t), e);
      }
    }
  },
  [["teuK", 1, 2, 0, 3, 5, 6]]
]);
